<template>
  ...
  <div v-if="isLoggedIn" class="w-100 d-flex page-content">
    ...
    <SideBar v-if="accountType === 'student'">
      <li>
        <router-link :to="{name: 'TakeExam'}" :class="{active: this.$route.path.slice(0,11) === '/take-exam/'}"><font-awesome-icon icon="file-signature" title="Take Quiz" /><span>Take Quiz</span></router-link>
      </li>
      <li>
    ...
    <SideBar v-else>
      <li>
        <router-link :to="{name: 'MyExams'}" :class="{active: this.$route.path.slice(0,10) === '/my-exams/'}"><font-awesome-icon icon="server" title="My Quizzes" /><span>My Quizzes</span></router-link>
      </li>
      <li>
